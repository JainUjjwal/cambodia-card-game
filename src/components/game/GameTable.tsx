...
import DrawCardModal from './DrawCardModal'; // Import the new modal
...
const GameTable = ({ 
  gameData, myPlayerId, knownCards, onAcknowledgePeek, showInitialPeek, 
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-br from-slate-800 via-slate-900 to-black p-2 md:p-4 text-white font-sans">
      ...
      <div className="w-full max-w-4xl mt-4 p-2 bg-slate-800 bg-opacity-50 rounded-lg flex justify-center gap-2 flex-wrap">
        <button onClick={onDrawCard} disabled={!isMyTurn} className="bg-cyan-600 hover:bg-cyan-700 text-white font-bold py-2 px-4 rounded-lg disabled:bg-slate-600 disabled:cursor-not-allowed">Draw Card</button>
        <button disabled={!isMyTurn} className="bg-teal-600 hover:bg-teal-700 text-white font-bold py-2 px-4 rounded-lg disabled:bg-slate-600 disabled:cursor-not-allowed">Take {topCard?.value}{topCard?.suit}</button>
        <button disabled={!isMyTurn} className="bg-yellow-600 hover:bg-yellow-700 text-white font-bold py-2 px-4 rounded-lg flex items-center gap-2 disabled:bg-slate-600 disabled:cursor-not-allowed"><Swords size={16} /> Snap!</button>
        <button disabled={!isMyTurn} className="bg-red-600 hover:bg-red-700 text-white font-bold py-2 px-4 rounded-lg flex items-center gap-2 disabled:bg-slate-600 disabled:cursor-not-allowed"><Shield size={16} /> Call Cambodia</button>
      </div>
