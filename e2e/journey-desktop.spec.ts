...
async function setupScriptedGame(browser: any, isMobile: boolean = false) {
  ...
  for (let i = 0; i < playerCount; i++) {
    const context = await browser.newContext({
      ...
      recordVideo: { dir: `test-results/videos/journey/desktop/player${i + 1}` }
    });
...
test('User Journey: Scripted 3-player game (Desktop POV P3)', async ({ browser }) => {
  ...
  await test.step('Round 2: Key Actions', async () => {
    // 2.1 P1: Black King
    await expect(p1.getByText(/Draw, Take/i)).toBeVisible();
    await p1.getByRole('button', { name: /^Draw$/i }).click();
    await p1.getByRole('button', { name: /Use Action/i }).click();
    ...
    await getHandCards(p3).nth(1).click();
    await p3.waitForTimeout(ACTION_DELAY);
  });
